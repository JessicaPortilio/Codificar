...
# Estágios do boneco da forca em diferentes fases do jogo
estagios = [
    '''
    +---+
    |   |
...
class ForcaJogo:
    def __init__(self, janela):
        self.janela = janela # Janela principal do Tkinter
        self.janela.title('Jogo da Forca') # Define o título da janela
        self.opcao = StringVar(value='frutas') # Variável para armazenar a opção de categoria escolhida
        self.palavra_escolhida = ''  # Palavra que será escolhida para o jogo
        self.palavra_normalizada = '' # Palavra escolhida, mas normalizada (sem acentos)
        self.tamanho_palavra_escolhida = 0 # Tamanho da palavra escolhida
        self.vidas = 6 # Número inicial de vidas
        self.display = [] # Lista que mostrará o progresso do jogador
        self.palpites = set() # Conjunto de palpites feitos pelo jogador
        self.primeira_janela() # Inicializa a primeira janela do jogo
        
    def primeira_janela(self):
        self.janela.geometry('500x400')  # Define o tamanho da janela
        self.frame_principal = Frame(self.janela)  # Cria um frame (container) principal
        self.frame_principal.pack()  # Adiciona o frame na janela
        ...
        # Informação sobre a escolha da categoria
        ...
        self.frame_radio = Frame(self.janela) # Cria um frame para os botões de opção
        self.frame_radio.pack(pady=5)
        
        # Botões de opção de categoria
        ...
        self.frame_botao = Frame(self.janela) # Cria um frame para o botão de iniciar
    ...
    def iniciar_jogo(self):
        ...
        self.palavra_escolhida = random.choice(lista) # Escolhe uma palavra aleatória da lista
        self.palavra_normalizada = nomrmalizar(self.palavra_escolhida) # Normaliza a palavra escolhida
        self.tamanho_palavra_escolhida = len(self.palavra_escolhida) # Obtém o tamanho da palavra escolhida
        self.display = [letra if letra == ' ' else '_' for letra in self.palavra_escolhida] # Cria a lista de exibição com underscores
        self.palpites.clear() # Limpa os palpites anteriores
        
        # Remove os frames anteriores para abrir a nova janela do jogo
